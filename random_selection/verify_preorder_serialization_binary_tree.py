...
class Solution:
    def isValidSerialization(self, preorder: str) -> bool:
        slots, prev = 1, None
        for c in preorder:
            # once node passed, reduce slots
            if c == ",":
                slots -= 1
                # no slots available
                if slots < 0:
                    return False
                # one non-empty node creates slots for 2 children
                if prev != "#":
                    slots += 2
            prev = c

        # last node needs to handled differently
        if c == "#":
            slots -= 1
        else:
            slots += 1
        # preorder traversal will be valid only if all available slots are used up
        return slots == 0
