...
solution.pickIndex(); // return 0. Since there is only one single element on the array the only option is to return
the first element.
Example 2:
...
class Solution:
    ...
    def __init__(self, w: List[int]):
        ...
        for weight in w:
            sum_ += weight
            self.prefix_sum.append(sum_)
        self.total_sum = sum_

    def pickIndex(self) -> int:
        # target = self.total_sum * self.prefix_sum[randint(0, len(self.prefix_sum) - 1)]
